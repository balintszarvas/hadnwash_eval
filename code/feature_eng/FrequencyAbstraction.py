    ...
    def abstract_frequency(self, data_table, columns, window_size, sampling_rate):
        ...
        for col in columns:
            ...
            data_table[col].rolling(
                window_size + 1, step=4).apply(self.find_fft_transformation)
            ...
            fft_df = pd.DataFrame(self.temp_list)
            fft_df.index = data_table.index[window_size::4][:len(fft_df)]
            fft_df.columns = collist

            data_table = pd.concat([data_table, fft_df], axis=1)
