...
class NumericalAbstraction:
    ...
    def rolling_mode(self, data):
        return data.mode().iloc[0] if not data.mode().empty else np.nan
    ...
    def aggregate_value(self,data, window_size, aggregation_function):
        ...
        window = window_size
        # print(f'Aggregating {aggregation_function} with window size {window_size} ({window})')
        # Compute the values and return the result.
        if aggregation_function == 'mean':
            return data.rolling(window, min_periods=window_size, step=4).mean() # skip every second worth of samples (250ms per sample)
        elif aggregation_function == 'max':
            return data.rolling(window, min_periods=window_size, step=4).max()
        elif aggregation_function == 'min':
            return data.rolling(window, min_periods=window_size, step=4).min()
        elif aggregation_function == 'median':
            return data.rolling(window, min_periods=window_size, step=4).median()
        elif aggregation_function == 'std':
            return data.rolling(window, min_periods=window_size, step=4).std()
            ...
            return data.rolling(window, min_periods=window_size, step=4).apply(lambda x: x.max() - x.min())
        elif aggregation_function == 'mode':
            return data.rolling(window, min_periods=window_size, step=4).apply(self.rolling_mode)        
        #TODO: add your own aggregation function here
        else:
            return np.nan
